...
export const ConnectBtn = ({setIsLoggedIn}: any) => {
  ...
  if (!isConnected) {
    return (
      <button
        className="bg-[#27A6E4] py-2 px-4 rounded-full shadow-lg cursor-pointer hover:bg-opacity-50"
        onClick={async () => {
  ...
  return (
    <div className="max-w-5xl w-full flex gap-4 items-center justify-between">
      ...
      <button className="" onClick={openChainModal}>
        Switch Networks
      </button>
    </div>
  );
